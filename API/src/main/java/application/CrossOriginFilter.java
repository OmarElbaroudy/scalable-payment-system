...
public class CrossOriginFilter implements Filter {
    ...
    public void init(FilterConfig config) throws ServletException {
        ...
        try {
            ...
            System.out.printf("Cross-origin filter, could not parse %s parameter as integer: %s", PREFLIGHT_MAX_AGE_PARAM, preflightMaxAgeConfig);
        }
        ...
        if (chainPreflightConfig != null)
            System.out.println("DEPRECATED CONFIGURATION: Use " + CHAIN_PREFLIGHT_PARAM + " instead of " + OLD_CHAIN_PREFLIGHT_PARAM);
        else
        ...
        if (origin != null && isEnabled(request)) {
            if (anyOriginAllowed || originMatches(allowedOrigins, origin)) {
                if (isSimpleRequest(request)) {
                    System.out.printf("Cross-origin request to %s is a simple cross-origin request", request.getRequestURI());
                    handleSimpleResponse(request, response, origin);
                } else if (isPreflightRequest(request)) {
                    System.out.printf("Cross-origin request to %s is a preflight cross-origin request", request.getRequestURI());
                    handlePreflightResponse(request, response, origin);
                    if (chainPreflight)
                        System.out.printf("Preflight cross-origin request to %s forwarded to application", request.getRequestURI());
                    else
                        return;
                } else {
                    System.out.printf("Cross-origin request to %s is a non-simple cross-origin request", request.getRequestURI());
                    handleSimpleResponse(request, response, origin);
                }
                ...
                if (anyTimingOriginAllowed || originMatches(allowedTimingOrigins, origin)) {
                    ...
                    System.out.println("Cross-origin request to " + request.getRequestURI() + " with origin " + origin + " does not match allowed timing origins " + allowedTimingOrigins);
                }
            } else {
    ...
    private boolean isMethodAllowed(HttpServletRequest request) {
        ...
        System.out.printf("%s is %s", ACCESS_CONTROL_REQUEST_METHOD_HEADER, accessControlRequestMethod);
        boolean result = false;
        if (accessControlRequestMethod != null)
            result = allowedMethods.contains(accessControlRequestMethod);
        System.out.printf("Method %s is" + (result ? "" : " not") + " among allowed methods %s", accessControlRequestMethod, allowedMethods);
        return result;
    }

    private List<String> getAccessControlRequestHeaders(HttpServletRequest request) {
        String accessControlRequestHeaders = request.getHeader(ACCESS_CONTROL_REQUEST_HEADERS_HEADER);
        System.out.printf("%s is %s", ACCESS_CONTROL_REQUEST_HEADERS_HEADER, accessControlRequestHeaders);
    ...
    private boolean areHeadersAllowed(List<String> requestedHeaders) {
        if (anyHeadersAllowed) {
            System.out.println("Any header is allowed");
            return true;
        }
        ...
        System.out.printf("Headers [{}] are" + (result ? "" : " not") + " among allowed headers {}", requestedHeaders, allowedHeaders);
        return result;
    }
